...
using ControlBooks.Migrations.SeedData;
...
namespace ControlBooks.Tests
{
    public abstract class ControlBooksTestBase : AbpIntegratedTestBase
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected ControlBooksTestBase()
        {
        ...
        protected override void AddModules(ITypeList<AbpModule> modules)
        {
            ...
            modules.Add<ControlBooksTestModule>();
        }
        ...
        protected void UsingDbContext(Action<ControlBooksDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<ControlBooksDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ControlBooksDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ControlBooksDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ControlBooksDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ControlBooksDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<ControlBooksDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ControlBooksDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ControlBooksDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ControlBooksDbContext, Task<T>> func)
        {
            T result;
